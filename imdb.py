...
if menu_id == 'Dashboard':
    ...
    ***
    ...
    table_genre1, table_genre2, table_genre3 = st.columns([1,2,2])
    with table_genre1:
        st.table(table)
    ...
    race1,race2 = st.columns([2,1])
    with race1:
        race = st.checkbox('Race Bar Chart')
        if race:
            video_file = open('https://github.com/willyrizkiyan/eda-and-imdb-rating-prediction/blob/main/IMDb%20Genre.mp4?raw=true', 'rb')
...
if menu_id == 'Quiz':
    ...
    ***
    ...
    if success == 'Avatar':
        ...
        data_sort_budget = data.sort_values('budget', ascending=False).reset_index(drop=True)
        data_sort_gross = data.sort_values('worldwide_gross', ascending=False).reset_index(drop=True)
        data_profit = data.copy()
        data_profit['profit'] = data_profit['worldwide_gross'] - data_profit['budget']
        data_sort_profit = data_profit.sort_values('profit', ascending=False).reset_index(drop=True)

        with bud1:
            data_sort_budget['budget'] = data_sort_budget.apply(lambda x: "{:,.0f}".format(x['budget']), axis=1)
            data_sort_budget.index = data_sort_budget.index + 1
            st.table(data_sort_budget[['title', 'budget']].head(10))

        with bud2:
            data_sort_gross['worldwide_gross'] = data_sort_gross.apply(lambda x: "{:,.0f}".format(x['worldwide_gross']), axis=1)
            data_sort_gross.index = data_sort_gross.index + 1
            st.table(data_sort_gross[['title', 'worldwide_gross']].head(10))

        with bud3:
            data_sort_profit['profit'] = data_sort_profit.apply(lambda x: "{:,.0f}".format(x['profit']), axis=1)
            data_sort_profit.index = data_sort_profit.index + 1            
            st.table(data_sort_profit[['title', 'profit']].head(10))
...
if menu_id == 'Summary':
    ...
    ***
    ...
    st.image('https://github.com/willyrizkiyan/eda-and-imdb-rating-prediction/blob/main/Feature%20Importance.png?raw=true', width=750)
...
if menu_id == 'Contact':
    ...
    ***
    ...
    I had worked in a mining company for almost 7 years. I made database and made simulation for production planning and product costing using previous data.
